    ...
    public partial class ThongKeSanPham : Form
    {
        ...
        private void cbx_sanpham_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = $"SELECT ID_sanpham, SUM(SoLuong) AS TongSoLuong FROM ChiTietHoaDon cthd, HoaDon hd WHERE cthd.ID_HoaDon = hd.ID_HoaDon and year(NgayXuat) = {cbx_sanpham.Text} GROUP BY ID_sanpham";
                DataSet ds = kn.selectData(query);
